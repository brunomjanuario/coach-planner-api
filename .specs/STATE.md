...
- **Feature**: `00-backend-mvp` — **Phases 0–3 done and verified, 28 tasks remain.** Spec/design/tasks written 2026-08-14; Phases 0–3 (T1–T21) implemented and committed the same day.
- **Phase / Task**: Phases 0, 1, 2 and 3 complete (T1–T21, all ✅ in `tasks.md`). Phase 4 (`T22`–`T26`, teams & players) is next — the first phase to build real domain endpoints on top of the auth/ownership foundation.
...
- Each task's own commit message documents the real discovery made while implementing it — read those before touching the same area again rather than re-deriving. **100 tests total, all green, stable across repeated runs.**
- **In-progress** (file:line): none — Phase 3 fully closed out.
...
- **Corrections made during Phase 3 execution** (the JWT/Security friction flagged as likely in the prior handoff did in fact materialize — five real findings, not typos):
  - `PasswordEncoder.encode()` and `Jwt.getSubject()` both surface as nullable (`String?`) to Kotlin despite never actually returning null in practice — resolved with `!!` at each call site rather than fighting the platform type.
  - `@ConditionalOnWebApplication` at the class level takes down *every* bean in that class for non-web test contexts (`FlywayMigrationIT`, `FailFastIT`), not just the one that needs `HttpSecurity` — the condition has to sit on the specific `@Bean` method (`filterChain`), never the enclosing `@Configuration` class. Hit twice (T14 for `passwordEncoder()`, T15 for `jwtDecoder()`) before the pattern was internalized.
  - **A real security bug, not a test artifact**: `RefreshTokenService.rotate()` is `@Transactional`, and reuse-detection's mass-revocation write ran inside that same transaction right before throwing — Spring's default rollback-on-unchecked-exception silently undid the revocation along with everything else. Fixed with a `TransactionTemplate(PROPAGATION_REQUIRES_NEW)`. General lesson: **any write that must survive an exception thrown later in the same method needs its own transaction boundary**, not a try/catch.
  - `.exceptionHandling { it.authenticationEntryPoint(...) }` does not work for JWT failures — `OAuth2ResourceServerConfigurer` registers its own `DelegatingAuthenticationEntryPoint` for bearer-token requests that takes precedence. The entry point has to be set via `.oauth2ResourceServer { it.authenticationEntryPoint(...) }` specifically.
  - Kotlin block comments **nest** (unlike Java/C). Writing `/api/v1/**` inside a KDoc comment's prose opened a second, never-closed comment level — the compiler error pointed at the file's last line, nowhere near the real cause. Any future comment mentioning a glob-style path should avoid the literal `/**`/`/*` substring.
- **Next step**: Phase 4 (`T22`–`T26`, teams & players) — `TeamController`/`PlayerController` CRUD, the team-delete cascade endpoint, position/stat validation, and extending `OwnershipIsolationIT`'s parameterised harness to the HTTP layer now that a real controller finally exists (the `404`-not-`403` claim becomes testable for the first time). This is a smaller, more mechanical phase than Phase 3 — no new framework surface, mostly wiring `OwnedRepository`/`CurrentUser` (both already built) into request handlers.
- **Blockers**: none. Docker running locally, Postgres 18 healthy, all Testcontainers-based tests green.
- **Uncommitted files**: none — working tree clean as of `87b665f`.
- **Branch**: `main`. All commits directly on `main`; no feature branch used for this round.
- **Open items carried forward**: (1) rival standings rows have no `team_id` in the frontend model, so a coach with two teams shares one league table — recorded as an assumption in `spec.md`, not resolved; (2) the frontend rewiring (replacing its 8 services with `fetch`) is explicitly out of scope this round and needs its own feature in the frontend repo; (3) no data import path from existing `localStorage` records — deliberately deferred; (4) no standalone `ExerciseRepository` yet — deferred to P11/T48 if/when granular exercise endpoints are built; (5) `OwnershipIsolationIT` currently only proves ownership scoping at the repository layer (no domain controllers existed when it was written) — Phase 4 should extend it to a real HTTP-level 404-not-403 check once `TeamController` exists, per its own doc comment's stated intent.
