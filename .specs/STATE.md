...
- **Feature**: `00-backend-mvp` — **Phases 0–2 done and verified, 36 tasks remain.** Spec/design/tasks written 2026-08-14; Phases 0–2 (T1–T13) implemented and committed the same day.
- **Phase / Task**: Phases 0, 1 and 2 complete (T1–T13, all ✅ in `tasks.md`). Phase 3 (`T14`–`T21`, auth & ownership) is next and not started — the largest remaining phase and the one everything downstream depends on.
...
- **Completed — Phase 2**: T11 (`common/errors.kt` — `NotFoundException`/`ValidationException`/`ConflictException`, each carrying an overridable `type` slug; `ApiExceptionHandler` as one `@RestControllerAdvice` building RFC 9457 `ProblemDetail` — `8237bf6`) → T12 (`MethodArgumentNotValidException` → 400 with a field-keyed `errors` extension member via `ProblemDetail.setProperty` — `ee811cb`) → T13 (`DataAccessResourceFailureException`→503, `OptimisticLockingFailureException`→409, `MethodArgumentTypeMismatchException`→400, catch-all `Exception`→500 with nothing leaked — `90df632`). All three tasks share one growing test class, `ApiExceptionHandlerIT` (9 tests), plus a `src/test`-only `ProblemTestController` fixture that's component-scanned automatically and never ships in the production jar.
- Each task's own commit message documents the real discovery made while implementing it — read those before touching the same area again rather than re-deriving. **59 tests total, all green, stable across repeated runs.**
- **In-progress** (file:line): none — Phase 2 fully closed out.
- **Corrections made during Phase 0 execution** (see git history for the full list — Kotlin 2.3.21 pin, Boot 4's per-technology autoconfigure split, Jackson 3 renames, Postgres 18's volume-mount change, `TestRestTemplate`/`HttpSecurity`/Logback gotchas). Still load-bearing.
- **Corrections made during Phase 1 execution** (see git history for the full list — `citext`/`smallint` exact-type requirements, `@OrderColumn`'s owning-side requirement, JSON `JdbcType` double-encoding risk, native-enum case sensitivity, testing DB-enforced cascades via raw JDBC not `repository.delete()`). Still load-bearing.
- **Corrections made during Phase 2 execution**: none — Phase 2 went green on the first attempt for every task, no surprises. (One environment interruption mid-Phase-2: the OS/sandbox briefly denied filesystem access to the whole project directory — resolved by the user granting the terminal app Full Disk Access and restarting it. Not a code issue; noted here only in case a future session sees the same symptom.)
- **Next step**: Phase 3 (`T14`–`T21`, auth & ownership) — registration, login, JWT issue/verify, refresh rotation with reuse detection, the security filter chain (replacing `SecurityConfig.kt`'s placeholder wholesale, not extending it), logout/profile, password change, and the `OwnedRepository` ownership-scoping pattern + isolation test harness. This is the largest remaining phase (8 tasks) and the one every later phase's endpoints depend on. Expect real friction here: this is the first phase touching JWT/Spring Security in earnest, an area Boot 4.1's module split has already been unpredictable in (T5's `SecurityConfig` + `@ConditionalOnWebApplication` lesson). Verify Spring Security 7's actual API surface (resource-server JWT decoder/encoder classes, filter chain builder DSL) against real jars before writing code, the same way Phase 0/1's Jackson 3 and Hibernate 7 surprises were handled.
- **Blockers**: none. Docker running locally, Postgres 18 healthy, all Testcontainers-based tests green.
- **Uncommitted files**: none — working tree clean as of `90df632`.
- **Branch**: `main`. All commits directly on `main`; no feature branch used for this round.
- **Open items carried forward**: (1) rival standings rows have no `team_id` in the frontend model, so a coach with two teams shares one league table — recorded as an assumption in `spec.md`, not resolved; (2) the frontend rewiring (replacing its 8 services with `fetch`) is explicitly out of scope this round and needs its own feature in the frontend repo; (3) no data import path from existing `localStorage` records — deliberately deferred; (4) `SecurityConfig.kt` is a deliberate placeholder (permits only `/actuator/health` and `/actuator/info`, denies everything else) — **Phase 3's T18 must replace it wholesale with the real JWT chain, not extend it incrementally**; (5) no standalone `ExerciseRepository` yet — deferred to P11/T48 if/when granular exercise endpoints are built.
