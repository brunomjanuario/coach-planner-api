...
- **Feature**: `00-backend-mvp` — **Phases 0–1 done and verified, 39 tasks remain.** Spec/design/tasks written 2026-08-14; Phase 0 (T1–T5) and Phase 1 (T6–T10) implemented and committed the same day.
- **Phase / Task**: Phases 0 and 1 complete (T1–T10, all ✅ in `tasks.md`). Phase 2 (`T11`–`T13`, error contract) is next and not started.
...
- **Completed — Phase 1**: T6 (the complete `V1__init.sql` in one pass; `SchemaIT` proves every table/CHECK/FK action against real Postgres via catalog queries — `a1a26a9`) → T7 (User/RefreshToken/Team/Player entities, hand-rolled UUIDv7 in `common/Ids.kt` — `935a364`) → T8 (Training/Exercise/Game/RivalRow entities, `Exercise.diagram` as `Map<String,Any?>?` jsonb — `bfbcf52`) → T9 (Card/Rating/Competition/Opponent entities, AD-106's two-nullable-FK rating model — `0542d25`) → T10 (`ConstraintsIT`/`CascadeIT` safety net, deletes via raw JDBC to prove DB-level enforcement, not Hibernate's own cascade config — `a4a83e9`). Each task's own commit message documents the real discovery made while implementing it — read those before touching the same area again rather than re-deriving. **50 tests total, all green, stable across repeated runs.**
- **In-progress** (file:line): none — Phase 1 fully closed out.
- **Corrections made during Phase 0 execution** (see prior handoff entry in git history for the full list — Kotlin 2.3.21 pin, Boot 4's per-technology autoconfigure split, Jackson 3 renames, Postgres 18's volume-mount change, `TestRestTemplate`/`HttpSecurity`/Logback gotchas). Still load-bearing for Phase 1+.
- **Corrections made during Phase 1 execution**:
  - `users.email` is `citext` in the schema; Hibernate defaults a Kotlin `String` column to `varchar(255)` — needed an explicit `columnDefinition = "citext"` or `ddl-auto=validate` fails immediately. **Any future `citext` column needs the same treatment.**
  - `@OrderColumn` only auto-populates the order value when the `@OneToMany` is the *owning* side. `Training.exercises` is the inverse side of `Exercise`'s `@ManyToOne` (`mappedBy`), and `exercises.training_id` is `NOT NULL` — Hibernate cannot write `order_index` from an inverse-side collection on the child's own insert (threw a `NOT NULL` violation on the first real insert). Fixed with `@OrderBy("orderIndex ASC")` over an explicit, caller-set `orderIndex` field on `Exercise` — the same pattern as `Team.players`' `@OrderBy(shirtNumber, name)`. **Any future ordered bidirectional `@OneToMany` should default to this pattern, not `@OrderColumn`.**
  - Hibernate's JSON `JdbcType` (`@JdbcTypeCode(SqlTypes.JSON)`) serializes whatever Kotlin type is given via a format mapper — mapping `Exercise.diagram` to `String` would double-encode it as a JSON string literal. Used `Map<String, Any?>?` instead; a typed diagram model is deliberately deferred to P5/T30.
  - `smallint` columns (`ratings.value`) need Kotlin `Short`, not `Int`, for `ddl-auto=validate` to pass — same class of issue as the `citext` one above; check exact column types, don't assume Kotlin's default numeric type matches.
  - Postgres native enum labels are matched by `@JdbcTypeCode(SqlTypes.NAMED_ENUM)` via exact `Enum.name()` string — no case transformation. `player_position`'s labels are uppercase (matches Kotlin convention), but `card_type`'s are lowercase (`'yellow'`,`'red'`), forcing `enum class CardType { yellow, red }` against Kotlin's normal `UPPER_CASE` convention. Documented inline so a future "cleanup" doesn't silently break it.
  - Testing DB-enforced cascades requires deleting via raw JDBC, not `repository.delete()` — several entities already carry Hibernate's own `cascade = ALL`, which produces identical observable results whether or not the database's FK action exists. `CascadeIT` deletes via `JdbcTemplate` specifically to prove the schema-level claim, not the ORM's.
  - Mixing raw JDBC writes with repository reads in the same `@DataJpaTest` needs an explicit `entityManager.clear()` after the JDBC write, or `findById` returns Hibernate's stale first-level-cache entity instead of re-querying.
- **Next step**: Phase 2 (`T11`–`T13`, error contract) — `ApiExceptionHandler` producing RFC 9457 `ProblemDetail`, bean-validation → field-keyed `errors`, and infrastructure failure mapping (`503`/`409`/`400` for bad UUIDs). Small phase, no entity/schema risk — should be fast relative to Phase 1.
- **Blockers**: none. Docker running locally, Postgres 18 healthy, all Testcontainers-based tests green.
- **Uncommitted files**: none — working tree clean as of `a4a83e9`.
- **Branch**: `main`. All commits directly on `main`; no feature branch used for this round.
- **Open items carried forward**: (1) rival standings rows have no `team_id` in the frontend model, so a coach with two teams shares one league table — recorded as an assumption in `spec.md`, not resolved; (2) the frontend rewiring (replacing its 8 services with `fetch`) is explicitly out of scope this round and needs its own feature in the frontend repo; (3) no data import path from existing `localStorage` records — deliberately deferred; (4) `SecurityConfig.kt` is a deliberate placeholder (permits only `/actuator/health` and `/actuator/info`, denies everything else) — Phase 3 (T14–T21) must replace it wholesale with the real JWT chain, not extend it incrementally; (5) no standalone `ExerciseRepository` yet — deferred to P11/T48 if/when granular exercise endpoints are built.
