...
- **Feature**: `00-backend-mvp` — **Phases 0–4 done and verified, 23 tasks remain.** Spec/design/tasks written 2026-08-14; Phases 0–4 (T1–T26) implemented and committed the same day.
- **Phase / Task**: Phases 0–4 complete (T1–T26, all ✅ in `tasks.md`). Phase 5 (`T27`–`T31`, trainings & exercises) is next.
...
- Each task's own commit message documents the real discovery made while implementing it — read those before touching the same area again rather than re-deriving. **118 tests total, all green, stable across repeated runs.**
- **In-progress** (file:line): none — Phase 4 fully closed out.
...
- **Next step**: Phase 5 (`T27`–`T31`, trainings & exercises) — training numbering (pure logic, ported from the frontend's `numberTrainings`), create/update with wholesale exercise replacement, diagram validation (shape-kind/size/count limits, coordinate clamping — the entity mapping for `Exercise.diagram` as `Map<String, Any?>?` already exists from T8), and delete-cascade. Should be mostly mechanical like Phase 4, with the numbering algorithm as the one piece worth extra care (equal-`day` tie-breaking, `teamId: null` → `number: null`, and the specific regression risk tasks.md flags: numbering computed *before* filtering by team, not after).
- **Blockers**: none. Docker running locally, Postgres 18 healthy, all Testcontainers-based tests green.
- **Uncommitted files**: none — working tree clean as of `00ec7e1`.
